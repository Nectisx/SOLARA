        ...
        for (const answer of data.answers) {
            ...
            if (sanitizedAnswer.trim().length < 10) {
                throw createError(
                    'Answer too short',
                    ErrorTypes.VALIDATION,
                    'Veuillez fournir des réponses détaillées (au moins 10 caractères).',
                    { length: sanitizedAnswer.length }
                );
            }
        }
    ...
    static checkApplicationCooldown(userId) {
        ...
        if (lastSubmit && now - lastSubmit < APPLICATION_SUBMIT_COOLDOWN) {
            ...
            throw createError(
                ...
                `Veuillez patienter ${Math.ceil(remainingTime / 60)} minute(s) avant de soumettre une nouvelle candidature.`,
        ...
        try {
            ...
            if (!['approve', 'deny'].includes(action)) {
                throw createError(
                    ...
                    ErrorTypes.VALIDATION,
                    "L'action de révision doit être soit approuver, soit refuser.",
                    { action }
                );
            }
        ...
        try {
            ...
            if (updates.logChannelId && typeof updates.logChannelId !== 'string') {
                throw createError(
                    ...
                    ErrorTypes.VALIDATION,
                    'Identifiant de salon invalide fourni.',
            ...
            if (updates.managerRoles && !Array.isArray(updates.managerRoles)) {
                throw createError(
                    ...
                    ErrorTypes.VALIDATION,
                    'Les rôles gestionnaires doivent être un tableau.',
            ...
            if (action === 'add') {
                ...
                if (!roleId) {
                    throw createError(
                        'Missing role ID',
                        ErrorTypes.VALIDATION,
                        'Vous devez spécifier un rôle à retirer.',
                        { action }
                    );
                }
    ...
    static async getUserApplications(client, guildId, userId) {
        try {
            ...
            throw createError(
                ...
                ErrorTypes.DATABASE,
                'Une erreur s\'est produite lors de la récupération de vos candidatures.',
                { guildId, userId }
            );
        }
    }
