    ...
    static async validateImageUrl(url) {
        ...
        try {
            ...
            throw createError(
                'Invalid image URL',
                ErrorTypes.VALIDATION,
                'L\'URL de l\'image doit être un lien http(s) public vers une image et ne peut pas pointer vers des hôtes locaux/privés.',
                { url }
            );
        }
    }
    ...
    static async setupWelcome(client, guildId, config, adminId) {
        ...
        if (!channel || !channel.isTextBased?.()) {
            throw createError(
                'Invalid channel',
                ErrorTypes.VALIDATION,
                'Le salon spécifié n\'existe pas ou n\'est pas un salon textuel.',
    ...
    static async addAutoRole(client, guildId, roleId, adminId) {
        ...
        if (role.position >= botHighestRole?.position) {
            ...
            throw createError(
                'Role too high',
                ErrorTypes.VALIDATION,
                "Je ne peux pas attribuer des rôles supérieurs à mon rôle le plus élevé.",
        ...
        if (existingRoles.includes(roleId)) {
            ...
            throw createError(
                'Duplicate role',
                ErrorTypes.VALIDATION,
                'Ce rôle est déjà configuré pour être attribué automatiquement.',
                { roleId }
            );
        }
        ...
        if (existingRoles.length >= this.MAX_ROLES_PER_GUILD) {
            ...
            throw createError(
                'Too many roles',
                ErrorTypes.VALIDATION,
                `Vous ne pouvez attribuer automatiquement que **${this.MAX_ROLES_PER_GUILD}** rôles au maximum.`,
    ...
    static async removeAutoRole(client, guildId, roleId, adminId) {
        ...
        if (!existingRoles.includes(roleId)) {
            ...
            throw createError(
                'Role not found',
                ErrorTypes.VALIDATION,
                'Ce rôle n\'est pas configuré pour être attribué automatiquement.',
                { roleId }
            );
        }
    ...
    static previewWelcomeMessage(messageTemplate, data) {
        ...
        try {
            ...
            throw createError(
                'Preview failed',
                ErrorTypes.DATABASE,
                'Impossible de générer l\'aperçu du message. Veuillez vérifier le format de votre message.',
                { error: error.message }
            );
        }
    }
    ...
    static async bulkUpdateAutoRoles(client, guildId, roleIds, adminId) {
        ...
        if (lastUpdate && (now - lastUpdate) < this.AUTOROLE_UPDATE_COOLDOWN) {
            ...
            throw createError(
                ...
                `Les mises à jour groupées sont limitées à une fois toutes les **5 minutes**. Attendez **${Math.ceil(remaining / 1000)}** seconde(s).`,
                { remaining, guildId }
            );
        }
