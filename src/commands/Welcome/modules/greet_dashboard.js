...
function buildDashboardEmbed(cfg, guild) {
    ...
    return new EmbedBuilder()
        .setTitle('👋 Tableau de bord du système de bienvenue')
        .setDescription(
            `Gérez les paramètres de bienvenue et d'au revoir pour **${guild.name}**.\nUtilisez les boutons pour activer/désactiver chaque côté, puis sélectionnez une option à modifier.`,
        )
        .setColor(getColor('info'))
        .addFields(
            { name: '🟢 Salon de bienvenue', value: welcomeChannel, inline: true },
            { name: '⚙️ Statut bienvenue', value: cfg.enabled ? '✅ Activé' : '❌ Désactivé', inline: true },
            { name: '🔔 Mention bienvenue', value: cfg.welcomePing ? '✅ On' : '❌ Off', inline: true },
            { name: '🔴 Salon Au revoir', value: goodbyeChannel, inline: true },
            { name: '⚙️ Statut au revoir', value: cfg.goodbyeEnabled ? '✅ Activé' : '❌ Désactivé', inline: true },
            { name: '🔔 Mention au revoir', value: cfg.goodbyePing ? '✅ On' : '❌ Off', inline: true },
    ...
    return new StringSelectMenuBuilder()
        ...
        .addOptions(
            ...
            new StringSelectMenuOptionBuilder()
                ...
                .setDescription('Modifier le texte affiché lorsqu\'un membre rejoint')
            ...
            new StringSelectMenuOptionBuilder()
                ...
                .setEmoji('🖼️'),
            new StringSelectMenuOptionBuilder()
                .setLabel('Salon Au revoir')
                .setDescription('Définir le salon où les messages d\'au revoir sont envoyés')
                .setValue('goodbye_channel')
                .setEmoji('🔴'),
            new StringSelectMenuOptionBuilder()
                .setLabel('Message Au revoir')
                .setDescription('Modifier le texte affiché lorsqu\'un membre quitte')
                .setValue('goodbye_message')
                .setEmoji('💬'),
            new StringSelectMenuOptionBuilder()
                .setLabel('Image Au revoir')
        ...
        try {
            ...
            if (!cfg.channelId && !cfg.goodbyeChannelId) {
                throw new TitanBotError(
                    ...
                    'Ni Bienvenue ni Au revoir n\'ont été configurés. Exécutez `/welcome setup` ou `/goodbye setup` d\'abord.',
                );
            }
            ...
            btnCollector.on('collect', async btnInteraction => {
                ...
                if (customId === `greet_cfg_toggle_welcome_${guildId}`) {
                    ...
                    await btnInteraction.followUp({
                        embeds: [
                            successEmbed(
                                '✅ Bienvenue mis à jour',
                                `Les messages de bienvenue sont maintenant **${cfg.enabled ? 'activés' : 'désactivés'}**.`,
                            ),
                        ],
                        flags: MessageFlags.Ephemeral,
                    });
                    ...
                    await btnInteraction.followUp({
                        embeds: [
                            successEmbed(
                                ...
                                `Les messages d'au revoir sont maintenant **${cfg.goodbyeEnabled ? 'activés' : 'désactivés'}**.`,
                            ),
                        ],
                        flags: MessageFlags.Ephemeral,
                    });
                    ...
                    await btnInteraction.followUp({
                        embeds: [
                            successEmbed(
                                '✅ Mention bienvenue mise à jour',
                                `Les utilisateurs rejoignant seront${cfg.welcomePing ? '' : ' **non**'} mentionnés dans le message de bienvenue.`,
                            ),
                        ],
                        flags: MessageFlags.Ephemeral,
                    });
                    ...
                    await btnInteraction.followUp({
                        embeds: [
                            successEmbed(
                                '✅ Mention au revoir mise à jour',
                                `Les utilisateurs partant seront${cfg.goodbyePing ? '' : ' **non**'} mentionnés dans le message d'au revoir.`,
                            ),
                        ],
                        flags: MessageFlags.Ephemeral,
                    });
            ...
            throw new TitanBotError(
                ...
                ErrorTypes.UNKNOWN,
                'Impossible d\'ouvrir le tableau de bord de bienvenue.',
            );
        }
    },
};
...
async function handleWelcomeChannel(selectInteraction, rootInteraction, cfg, guildId, client) {
    ...
    chanCollector.on('end', (collected, reason) => {
        if (reason === 'time' && collected.size === 0) {
            selectInteraction
                .followUp({
                    embeds: [errorEmbed('Délai expiré', 'Aucun salon n\'a été sélectionné. Le paramètre n\'a pas été modifié.')],
                    flags: MessageFlags.Ephemeral,
                })
                .catch(() => {});
        }
...
async function handleWelcomeMessage(selectInteraction, rootInteraction, cfg, guildId, client) {
    ...
    await submitted.reply({
        embeds: [successEmbed('✅ Message de bienvenue mis à jour', 'Le message de bienvenue a été sauvegardé.')],
        flags: MessageFlags.Ephemeral,
    });
...
async function handleWelcomeImage(selectInteraction, rootInteraction, cfg, guildId, client) {
    ...
    const imageHint = new TextDisplayBuilder()
        .setContent('Fournissez une URL d\'image directe **ou** téléchargez un fichier ci-dessous. Si les deux sont fournis, le fichier téléchargé est prioritaire. Laissez l\'URL vide et ignorez le téléchargement pour supprimer l\'image.');
...
async function handleGoodbyeChannel(selectInteraction, rootInteraction, cfg, guildId, client) {
    ...
    await selectInteraction.followUp({
        embeds: [
            new EmbedBuilder()
                .setTitle('🔴 Salon Au revoir')
                .setDescription(
                    `**Actuel :** ${cfg.goodbyeChannelId ? `<#${cfg.goodbyeChannelId}>` : '`Non défini`'}\n\nSélectionnez le salon où les messages d'au revoir seront envoyés.`,
                )
                .setColor(getColor('info')),
        ],
        components: [new ActionRowBuilder().addComponents(channelSelect)],
    ...
    chanCollector.on('collect', async chanInteraction => {
        ...
        await chanInteraction.followUp({
            embeds: [successEmbed('✅ Salon mis à jour', `Les messages d'au revoir seront désormais envoyés dans ${channel}.`)],
            flags: MessageFlags.Ephemeral,
        });
...
async function handleGoodbyeMessage(selectInteraction, rootInteraction, cfg, guildId, client) {
    ...
    await submitted.reply({
        embeds: [successEmbed('✅ Message d\'au revoir mis à jour', 'Le message d\'au revoir a été sauvegardé.')],
        flags: MessageFlags.Ephemeral,
    });
