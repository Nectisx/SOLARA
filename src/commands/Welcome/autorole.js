        ...
        if (subcommand === 'add') {
            ...
            if (verificationEnabled || autoVerifyEnabled) {
                return InteractionHelper.safeEditReply(interaction, {
                    embeds: [errorEmbed(
                        ...
                        'Vous ne pouvez pas ajouter un AutoRole tant que le système de vérification ou AutoVerify est activé. Désactivez-les d\'abord.'
                    )],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (role.position >= guild.members.me.roles.highest.position) {
                ...
                return InteractionHelper.safeReply(interaction, {
                    embeds: [errorEmbed('Rôle trop élevé', "Je ne peux pas attribuer des rôles supérieurs à mon rôle le plus élevé.")],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            try {
                ...
                if (currentRoleId === role.id) {
                    ...
                    return InteractionHelper.safeEditReply(interaction, {
                        embeds: [errorEmbed('Déjà ajouté', `Le rôle ${role} est déjà défini pour être attribué automatiquement.`)],
                        flags: MessageFlags.Ephemeral
                    });
                }
                ...
                await InteractionHelper.safeEditReply(interaction, {
                    embeds: [errorEmbed(
                        'Ajout échoué',
                        'Une erreur s\'est produite lors de l\'ajout du rôle. Veuillez réessayer.',
                        { showDetails: true }
                    )],
                    flags: MessageFlags.Ephemeral
                });
            ...
            try {
                ...
                if (!existingRoles.includes(role.id)) {
                    ...
                    return InteractionHelper.safeEditReply(interaction, {
                        embeds: [errorEmbed('Introuvable', `Le rôle ${role} n\'est pas défini pour être attribué automatiquement.`)],
                        flags: MessageFlags.Ephemeral
                    });
                }
                ...
                await InteractionHelper.safeEditReply(interaction, {
                    embeds: [errorEmbed(
                        'Suppression échouée',
                        'Une erreur s\'est produite lors de la suppression du rôle. Veuillez réessayer.',
                        { showDetails: true }
                    )],
                    flags: MessageFlags.Ephemeral
                });
            ...
            try {
                ...
                if (singleRoleIds.length === 0) {
                    return InteractionHelper.safeEditReply(interaction, {
                        embeds: [createAutoroleInfoEmbed(`ℹ️ Aucun rôle n'est défini pour être attribué automatiquement.${conflictSummary ? `\n\n⚠️ Blocages de configuration :\n${conflictSummary}` : ''}`)],
                        flags: MessageFlags.Ephemeral
                    });
                }
                ...
                if (validRoles.length === 0) {
                    return InteractionHelper.safeEditReply(interaction, {
                        embeds: [createAutoroleInfoEmbed(`ℹ️ Aucun rôle automatique valide trouvé. Tout rôle invalide a été supprimé.${conflictSummary ? `\n\n⚠️ Blocages de configuration :\n${conflictSummary}` : ''}`)],
                        flags: MessageFlags.Ephemeral
                    });
                }
                ...
                await InteractionHelper.safeEditReply(interaction, {
                    embeds: [errorEmbed(
                        'Listage échoué',
                        'Une erreur s\'est produite lors de la liste des rôles attribués automatiquement. Veuillez réessayer.',
                        { showDetails: true }
                    )],
                    flags: MessageFlags.Ephemeral
                });
