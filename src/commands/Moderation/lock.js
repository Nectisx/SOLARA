    ...
    if (!interaction.member.permissions.has(PermissionFlagsBits.ManageChannels))
      return await InteractionHelper.safeEditReply(interaction, {
        embeds: [
          errorEmbed(
            "Permission refusée",
            "Vous avez besoin de la permission `Gérer les salons` pour verrouiller les salons.",
          ),
        ],
      });
...
{ type: 0, reason: `Channel locked by ${interaction.user.tag}` },
      ...
      const lockEmbed = createEmbed(
        "🔒 Salon verrouillé (Journal d'action)",
...
.setColor(getColor('moderation'))
      ...
      await InteractionHelper.safeEditReply(interaction, {
        embeds: [
          successEmbed(
            `🔒 **Salon verrouillé**`,
            `${channel} est maintenant verrouillé. Personne ne peut parler ici.`,
          ),
        ],
      });
    } catch (error) {
      ...
      await InteractionHelper.safeEditReply(interaction, {
        embeds: [
          errorEmbed(
            "Une erreur inattendue s'est produite lors du verrouillage du salon. Vérifiez mes permissions (j'ai besoin de 'Gérer les salons').",
          ),
        ],
      });
    }
