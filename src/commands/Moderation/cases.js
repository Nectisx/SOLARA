    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('filter')
                .setDescription('Filtrer les cas par type ou utilisateur')
                .addChoices(
                    { name: 'Tous les cas', value: 'all' },
                    { name: 'Bannissements', value: 'Member Banned' },
                    { name: 'Expulsions', value: 'Member Kicked' },
                    { name: 'Mises en sourdine', value: 'Member Timed Out' },
                    { name: 'Avertissements', value: 'User Warned' }
                )
        )
        .addUserOption(option =>
            option.setName('user')
                .setDescription('Filtrer les cas par utilisateur spécifique')
        )
        ...
        try {
            ...
            if (cases.length === 0) {
                throw new Error(targetUser
                    ...
                    : `Aucun cas${filterType === 'all' ? '' : ' de type ' + filterType} trouvé dans ce serveur.`
                );
            }
            ...
            const createCasesEmbed = (page) => {
                ...
                const embed = createEmbed({
                    ...
                    description: `Cas de modération pour **${interaction.guild.name}**\n\n**Page ${page} sur ${totalPages}**`
                });
...
time: 120000
            ...
            collector.on('collect', async (buttonInteraction) => {
                ...
                if (buttonInteraction.user.id !== interaction.user.id) {
                    await buttonInteraction.followUp({
                        content: 'Vous ne pouvez pas utiliser ces boutons. Exécutez `/cases` pour obtenir votre propre vue des cas.',
            ...
            return InteractionHelper.safeEditReply(interaction, {
                embeds: [
                    errorEmbed(
                        'Erreur système',
                        'Une erreur s\'est produite lors de la récupération des cas de modération. Veuillez réessayer plus tard.'
                    )
                ],
                flags: MessageFlags.Ephemeral
            });
