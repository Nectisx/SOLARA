...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName("reason")
                .setDescription("Raison du bannissement en masse")
                .setRequired(false)
        )
    ...
    async execute(interaction, config, client) {
        ...
        const reason = interaction.options.getString("reason") || "Bannissement en masse - Aucune raison fournie";
        ...
        try {
            ...
            if (!isAllowed) {
                return await InteractionHelper.safeEditReply(interaction, {
                    embeds: [
                        warningEmbed(
                            "Vous effectuez des bannissements en masse trop rapidement. Veuillez attendre une minute avant de réessayer.",
...
.slice(0, 20);
                ...
                return await InteractionHelper.safeEditReply(interaction, {
                    embeds: [
                        errorEmbed(
                            ...
                            "Vous ne pouvez pas vous inclure dans un bannissement en masse."
                        ),
                    ],
                });
            }
                ...
                return await InteractionHelper.safeEditReply(interaction, {
                    embeds: [
                        errorEmbed(
                            ...
                            "Vous ne pouvez pas inclure le bot dans un bannissement en masse."
                        ),
                    ],
                });
            }
                    ...
                    if (member) {
                        if (member.roles.highest.position >= interaction.member.roles.highest.position && 
                            ...
                            results.skipped.push({ 
                                user: user.tag, 
                                userId, 
                                reason: "Impossible de bannir un utilisateur avec un rôle égal ou supérieur"
                            });
                            continue;
                        }
                    }
            ...
            let description = `**Résultats du bannissement en masse :**\n\n`;
            ...
            return await InteractionHelper.safeEditReply(interaction, {
                embeds: [
                    errorEmbed(
                        "Erreur système",
                        "Une erreur s'est produite lors du traitement du bannissement en masse. Veuillez réessayer plus tard."
                    ),
                ],
            });
        }
