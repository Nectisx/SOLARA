...
async function handleSetup(interaction) {
    ...
    const roleSelect = new RoleSelectMenuBuilder()
        .setCustomId('role_id')
        .setPlaceholder('Sélectionner le rôle pour lequel les utilisateurs postulent')
        .setRequired(true);

    const roleLabel = new LabelBuilder()
        .setLabel('Rôle de la candidature')
        .setDescription('Le rôle pour lequel les utilisateurs postulent')
    ...
    await submitted.reply({
        embeds: [successEmbed(
            '✅ Candidature créée',
            `La candidature **${appName}** a été créée pour ${role}.\n\nVous pouvez personnaliser le salon de logs, les rôles gestionnaires, les questions et la période de rétention dans le tableau de bord.`,
        )],
        flags: ['Ephemeral'],
    });
...
async function handleReview(interaction) {
    ...
    collector.on('collect', async buttonInteraction => {
        ...
        const reasonModal = new ModalBuilder()
            ...
            .setTitle(`${isApprove ? 'Approuver' : 'Refuser'} la candidature - Raison`);
        ...
        reasonModal.addComponents(
            new ActionRowBuilder().addComponents(
                new TextInputBuilder()
                    ...
                    .setPlaceholder('Fournissez une raison pour cette décision...')
                    .setMaxLength(500)
                    .setRequired(false),
            ),
        );
        ...
        try {
            ...
            const reason = reasonSubmit.fields.getTextInputValue('review_reason').trim() || "Aucune raison fournie.";
            ...
            try {
                ...
                const dmEmbed = createEmbed(
                    ...
                    `Votre candidature pour **${application.roleName}** a été **${status}**\n` +
                        `**Note :** ${reason}\n\n` +
            ...
            await buttonInteraction.reply({
                embeds: [errorEmbed('Erreur', 'Une erreur s\'est produite lors de l\'examen de la candidature.')],
                flags: ["Ephemeral"],
            });
        }
    });
            ...
            return InteractionHelper.safeEditReply(interaction, {
                embeds: [
                    errorEmbed(
                        ...
                        "Utilisez `/app-admin roles add` pour configurer les rôles de candidature en premier."
                    ),
                ],
                flags: ["Ephemeral"],
            });
...
export async function handleApplicationReviewModal(interaction) {
    ...
    try {
        ...
        await InteractionHelper.safeEditReply(interaction, {
            embeds: [errorEmbed('Une erreur s\'est produite lors du traitement de la candidature.')],
            flags: ["Ephemeral"]
        });
    }
}
