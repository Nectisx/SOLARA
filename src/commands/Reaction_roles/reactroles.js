...
export default {
    data: new SlashCommandBuilder()
        .setName('reactroles')
        .setDescription('Gérer les attributions de rôles par réaction')
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addStringOption(option =>
                    option
                        .setName('panel')
                        .setDescription('Sélectionnez un panneau de rôles par réaction à gérer')
                        .setRequired(false)
...
async function handleSetup(interaction) {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        if (role) {
            if (role.position >= interaction.guild.members.me.roles.highest.position) {
                roleValidationErrors.push(`**${role.name}** - Le rôle de mon bot est positionné en dessous de ce rôle dans la hiérarchie du serveur et ne peut pas l'attribuer`);
                continue;
            }
    ...
    const panelEmbed = new EmbedBuilder()
        ...
        .setFooter({ text: 'Sélectionnez des rôles dans le menu déroulant ci-dessous' });
    ...
    collector.on('end', async (_, reason) => {
        ...
        if (reason === 'time') {
            const timeoutEmbed = new EmbedBuilder()
                ...
                .setDescription('Cette session de tableau de bord a expiré en raison d\'une inactivité (10 minutes).\n\nPour continuer à gérer vos rôles par réaction, veuillez relancer `/reactroles dashboard`.')
...
async function rebuildLivePanelMessage(guild, panelData) {
    try {
        ...
        const roleFieldIdx = fields.findIndex(f => f.name === 'Rôles disponibles' || f.name === 'Available Roles');
    ...
    const optionsSelect = new StringSelectMenuBuilder()
        ...
        .addOptions(
            new StringSelectMenuOptionBuilder()
                ...
                .setDescription('Ajouter un rôle à ce panneau (25 maximum)')
                .setValue('add_role')
                .setEmoji('➕'),
            ...(panelData.roles.length > 0 ? [
                new StringSelectMenuOptionBuilder()
...
async function handleEditText(buttonInteraction, rootInteraction, panelData, guildId, guild, client) {
    ...
    try {
        ...
        await buttonInteraction.followUp({
            embeds: [errorEmbed('Erreur', 'Impossible d\'afficher le modal de modification du texte du panneau. Veuillez réessayer.')],
...
async function handleAddRole(selectInteraction, rootInteraction, panelData, guildId, guild, client) {
    ...
    if (panelData.roles.length >= 25) {
        await selectInteraction.followUp({
            embeds: [errorEmbed('Panneau plein', 'Ce panneau contient déjà le maximum de 25 rôles.')],
            flags: MessageFlags.Ephemeral,
        });
        return;
    }
    ...
    roleCollector.on('collect', async roleInteraction => {
        ...
        if (role.managed) {
            await roleInteraction.followUp({
                embeds: [errorEmbed('Rôle invalide', 'Les rôles gérés/de bot ne peuvent pas être utilisés.')],
                flags: MessageFlags.Ephemeral,
            });
            return;
        }
        ...
        if (role.position >= guild.members.me.roles.highest.position) {
            await roleInteraction.followUp({
                embeds: [
                    errorEmbed(
                        'Rôle trop élevé',
                        "Ce rôle est au-dessus de mon rôle le plus élevé dans la hiérarchie. Déplacez mon rôle au-dessus d'abord.",
                    ),
                ],
                flags: MessageFlags.Ephemeral,
            });
    ...
    roleCollector.on('end', (collected, reason) => {
        if (reason === 'time' && collected.size === 0) {
            selectInteraction
                .followUp({
                    embeds: [errorEmbed('Délai expiré', 'Aucun rôle sélectionné. Aucune modification effectuée.')],
                    flags: MessageFlags.Ephemeral,
                })
                .catch(() => {});
        }
...
async function handleDeletePanel(btnInteraction, rootInteraction, panelData, panels, guildId, guild, client, collector, buttonCollector) {
    ...
    const deleteWarningText = new TextDisplayBuilder()
        .setContent(`⚠️ Vous êtes sur le point de supprimer définitivement le panneau **${title}**. Cela supprimera le message Discord et toutes les attributions de rôles par réaction associées.`);
