...
export const counterDeleteActionHandler = {
  ...
  async execute(interaction, client, args = []) {
    try {
      ...
      if (!action || !counterId) {
        await interaction.editReply({
          embeds: [errorEmbed('Action invalide', 'Les données de l\'action de suppression du compteur sont manquantes.')],
          components: []
        }).catch(logger.error);
        return;
      }
      ...
      if (ownerId && interaction.user.id !== ownerId) {
        await interaction.editReply({
          embeds: [errorEmbed('Non autorisé', 'Seul l\'utilisateur qui a initié cette suppression peut utiliser ces boutons.')],
          components: []
        }).catch(logger.error);
        return;
      }
      ...
      if (action !== 'confirm') {
        await interaction.editReply({
          embeds: [errorEmbed('Action invalide', 'Action de suppression du compteur inconnue.')],
          components: []
        }).catch(logger.error);
        return;
      }
      ...
      if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({
          embeds: [errorEmbed('Erreur', 'Une erreur s\'est produite lors du traitement de cette action.')],
